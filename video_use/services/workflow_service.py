...
"""
        ...
        try:
            messages = [
                SystemMessage(
                    content=STRUCTURED_WORKFLOW_PROMPT.format(
            ...
            logger.info(
                f"Generated structured workflow with {len(result.parameters)} parameters (tokens: {token_usage.total_tokens})"
            )
            return result
    ...
    async def convert_gemini_analysis_to_structured_output(
        self, gemini_analysis_text: str, start_url: Optional[str] = None
    ) -> StructuredWorkflowOutput:
...
"""
        ...
        try:
            messages = [
                SystemMessage(
                    content=GEMINI_TO_STRUCTURED_PROMPT.format(
                        format_instructions=parser.get_format_instructions()
                    )
                ),
                HumanMessage(content=human_prompt),
            ...
            logger.info(
                f"Generated structured workflow from Gemini analysis with {len(result.parameters)} parameters (tokens: {token_usage.total_tokens})"
            )
            return result

        except Exception as e:
            logger.error(
                f"Error generating structured output from Gemini analysis: {e}"
            )
            return StructuredWorkflowOutput(
